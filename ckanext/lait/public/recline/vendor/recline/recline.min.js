...
template_it:' \
  <div class="editor"> \
    <form class="form-stacked"> \
      ...
      <div class="editor-buttons"> \
        <button class="btn editor-add">Aggiungi Serie</button> \
      </div> \
      <div class="editor-buttons editor-submit" comment="hidden temporarily" style="display: none;"> \
...
,
  ...
  ',
    ...
    <div class="editor-series js-series-{{seriesIndex}}"> \
      <label>Series <span>{{seriesName}} (Asse 2)</span> \
...
my.MapMenu = Backbone.View.extend({
  ...
  template_it: ' \
    <form class="form-stacked"> \
      ...
      <div class="editor-buttons"> \
        <button class="btn editor-update-map">Aggiorna</button> \
      </div> \
      <div class="editor-options" > \
        <label class="checkbox"> \
          <input type="checkbox" id="editor-auto-zoom" value="autozoom" checked="checked" /> \
...
my.MultiView = Backbone.View.extend({
  ...
  initialize: function(options) {
    ...
    for (i = 0; i < this.pageViews.length; i++) { 
	    if(this.pageViews[i].label=='Grid')
	    	this.pageViews[i].label='Griglia';
	    else if(this.pageViews[i].label=='Graph')
	    	this.pageViews[i].label='Grafico';
	    else if(this.pageViews[i].label=='Map')
    ...
    for (i = 0; i < this.sidebarViews.length; i++) { 
	    if(this.sidebarViews[i].label=='Filters')
	    	this.sidebarViews[i].label='Filtri';
	    else if(this.sidebarViews[i].label=='Fields')
...
my.FilterEditor = Backbone.View.extend({
  ...
  template_it: ' \
    <div class="filters"> \
      <h3>Filters</h3> \
      <a href="#" class="js-add-filter">Aggiungi filtro</a> \
      <form class="form-stacked js-add" style="display: none;"> \
        <fieldset> \
          <label>Campo</label> \
          <select class="fields"> \
          ...
          <select class="filterType"> \
            <option value="term">Valore</option> \
            <option value="range">Range</option> \
    ...
    term: ' \
      <div class="filter-{{type}} filter"> \
        <fieldset> \
          <legend> \
            {{field}} <small>{{type}}</small> \
            <a class="js-remove-filter" href="#" title="Rimuovi questo filtro" data-filter-id="{{id}}">&times;</a> \
          </legend> \
          <input type="text" value="{{term}}" name="term" data-filter-field="{{field}}" data-filter-id="{{id}}" data-filter-type="{{type}}" /> \
        </fieldset> \
      </div> \
  ...
  template: ' \
    <div class="pagination"> \
      <ul> \
        ...
        <li class="active"><a><input name="from" type="text" value="{{from}}" disabled /> &ndash; <input name="to" type="text" value="{{to}}" disabled /> </a></li> \
...
my.QueryEditor = Backbone.View.extend({
  ...
  template_it: ' \
    <form action="" method="GET" class="form-inline"> \
      <div class="input-prepend text-query"> \
        <span class="add-on"><i class="icon-search"></i></span> \
        <input type="text" name="q" value="{{q}}" class="span2" placeholder="Cerca..." class="search-query" /> \
      </div> \
      <button type="submit" class="btn">Vai &raquo;</button> \
    </form> \
  ',
