...
L.WFST = L.GeoJSON.extend({
    ...
    initialize: function(geojson,options){
        ...
        if(this.options.showExisting){
            this._loadExistingFeatures(this.options.wfsFilter);
        }
    ...
    */
    _loadExistingFeatures: function(wfsFilter){
        var geoJsonUrl = this.options.url + '?service=WFS&version=' + this.options.version + '&request=GetFeature&typeName=' + this.options.featureNS + ':' + this.options.featureType + '&srsName=epsg:4326&outputFormat=application/json';
        if (wfsFilter != null)
        {
            geoJsonUrl += "&filter="+wfsFilter;
        }
        this._ajax({
            url: geoJsonUrl,
            success: function(res){
